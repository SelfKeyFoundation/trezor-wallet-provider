...
var _msgConfig = _interopRequireDefault(require("./msg-config"));
...
class TrezorWallet {
  ...
  async signTransactionAsync(txData) {
    ...
    let signPromise = session.signEthTx(this._getAddressByIndex(accountIndex), txDataClone.nonce, txDataClone.gasPrice, txDataClone.gasLimit, txDataClone.to, txDataClone.value, txDataClone.data, this.networkId);
    let signed = null;
    ...
    const signedTx = new _ethereumjsTx.Transaction({
